...
async function captureScreenshot(type, gw, userToken, teamId = null) {
  ...
  try {
    ...
    // 1. تحديد العرض بناءً على النوع (كما في الكود الأول المثالي)
    ...
    // ✅ حقن التوكن (استخدام طريقة evaluateOnNewDocument من الكود الأول لضمان المصادقة)
    if (userToken) {
    ...
    // ✅ بناء الرابط الصحيح (من الكود الأول)
    let targetUrl;
    ...
    // ✅ الانتظار لتحميل المحتوى (كما في الكود الأول)
    await page.waitForSelector('body', { timeout: 10000 });
    await new Promise(r => setTimeout(r, 3000));

    // 2. تطبيق الأنماط (دمج تنسيقات الكود الأول المثالية حرفياً)
    await page.evaluate((contentType) => {
      ...
      let finalCSS = `
        ...
        [class*="back-btn"], header, [class*="header"] { 
          display: none !important; 
        }
        [class*="testConnection"], [class*="info"], [class*="message"], .tournament-header {
          display: none !important;
        }
      `;
      ...
      if (contentType === 'league' || contentType === 'standings') {
        ...
        finalCSS += `
          ...
          .fixtures-container, [class*="match"], .match-row { 
...
// ✅ دالة التقاط الـ 20 تشكيلة (من الكود الثاني)
async function captureAllLineups(gw, userToken) {
  ...
  for (const teamId of allTeamIds) {
    try {
      const imageBuffer = await captureScreenshot('lineups', gw, userToken, teamId);
...
// ✅ دالة النشر الموحدة (تدعم الألبوم للتشكيلات والنشر الفردي للبقية)
exports.publishToFacebook = async (req, res) => {
  ...
  try {
    ...
    // النشر العادي (ترتيب/مباريات) كما في الكود الأول
